...
lv_obj_t *zmk_display_status_screen() {
  ...
  lv_obj_set_style_bg_color(
      left_panel, lv_color_hex(DISPLAY_COLOR_LEFT_PANEL_BG), LV_PART_MAIN);
  ...
  lv_obj_set_pos(zmk_widget_layer_indicator_obj(&layer_indicator_widget), 14,
                 20);
  ...
  lv_obj_align(zmk_widget_modifier_indicator_obj(&modifier_indicator_widget),
               LV_ALIGN_BOTTOM_RIGHT, 0, -62);

  zmk_widget_battery_circles_init(&battery_circles_widget, screen);
  lv_obj_set_pos(zmk_widget_battery_circles_obj(&battery_circles_widget), 172,
                 178);

  zmk_widget_recording_indicator_init(&recording_indicator_widget, screen);
  lv_obj_align(zmk_widget_recording_indicator_obj(&recording_indicator_widget),
